...
```go
import "github.com/luciancaetano/knet/timing"
...
A `TickFn` returning `nil` skips the broadcast for that tick — use this for delta strategies where nothing changed.
